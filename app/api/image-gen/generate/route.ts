...
import { startTransaction, setAttributes, captureServerError } from '@/lib/new-relic'
...
export async function POST(request: Request) {
  return startTransaction('generate', 'ImageGeneration', async () => {
    try {
      // Authenticate User
      ...
      setAttributes({
        imageGenModel: _model,
        imageGenAspectRatio: aspect_ratio,
      ...
      try {
        ...
        // Add request ID to transaction for correlation
        if (backendResponse.request_id) {
          setAttributes({
            imageGenRequestId: backendResponse.request_id,
          })
        }
        ...
        captureServerError(backendError as Error, {
          context: 'imageGen.backend',
          userId: user.sub,
          model: _model,
        })
